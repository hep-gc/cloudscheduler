...
def add(request):
    ...
    if request.method == 'POST':
        ...
        if rc != 0:
            config.db_close()
            return list(request, selector='-', response_code=1, message='%s %s' % (lno('GV00'), msg), user_groups=user_groups)
        ...
        if rc != 0:
            config.db_close()
            return list(request, selector='-', response_code=1, message='%s group add %s' % (lno('GV01'), msg), user_groups=user_groups)
        ...
        if 'vm_flavor' in fields and fields['vm_flavor']:
            ...
            if rc != 0:
                config.db_close()
                return list(request, selector='-', response_code=1, message='%s group add, "%s" failed - %s.' % (lno('GV96'), fields['group_name'], msg), user_groups=user_groups)
        ...
        if 'vm_image' in fields and fields['vm_image']:
            ...
            if rc != 0:
                config.db_close()
                return list(request, selector='-', response_code=1, message='%s group add, "%s" failed - %s.' % (lno('GV97'), fields['group_name'], msg), user_groups=user_groups)
        ...
        if 'vm_keyname' in fields and fields['vm_keyname']:
            ...
            if rc != 0:
                config.db_close()
                return list(request, selector='-', response_code=1, message='%s group add, "%s" failed - %s.' % (lno('GV95'), fields['group_name'], msg), user_groups=user_groups)
        ...
        if 'username' in fields:
            ...
            if rc != 0:
                config.db_close()
                return list(request, selector=fields['group_name'], response_code=1, message='%s group add, "%s" failed - %s.' % (lno('GV02'), fields['group_name'], msg), user_groups=user_groups)
        ...
        if rc != 0:
            config.db_close()
            return list(request, selector=fields['group_name'], response_code=1, message='%s group add "%s" failed - %s.' % (lno('GV03'), fields['group_name'], msg), user_groups=user_groups, attributes=columns)
        ...
        if rc == 0:
            ...
            return list(request, selector=fields['group_name'], response_code=0, message='group "%s" successfully added.' % (fields['group_name']), user_groups=user_groups, attributes=columns)
        else:
            config.db_close()
            return list(request, selector=fields['group_name'], response_code=1, message='%s group defaults add "%s" failed - %s.' % (lno('GV04'), fields['group_name'], msg), user_groups=user_groups, attributes=columns)
...
def delete(request):
    ...
    if request.method == 'POST':
        ...
        if rc != 0:
            config.db_close()
            return list(request, selector='-', response_code=1, message='%s %s' % (lno('GV09'), msg), user_groups=user_groups)
        ...
        if rc != 0:
            config.db_close()
            return list(request, selector='-', response_code=1, message='%s group delete %s' % (lno('GV10'), msg), user_groups=user_groups)
        ...
        for row in group_list:
            if row['group_name'] == fields['group_name'] and row['metadata_names']:
                ...
                for metadata_name in metadata_names:
                    ...
                    if rc != 0:
                        config.db_close()
                        return list(request, selector=fields['group_name'], response_code=1, message='%s group metadata file delete "%s::%s" failed - %s.' % (lno('GV11'), fields['group_name'], metadata_name, msg), user_groups=user_groups, attributes=columns)
        ...
        if rc != 0:
            config.db_close()
            return list(request, selector=fields['group_name'], response_code=1, message='%s group defaults delete "%s" failed - %s.' % (lno('GV12'), fields['group_name'], msg), user_groups=user_groups, attributes=columns)
        ...
        if rc != 0:
            config.db_close()
            return list(request, selector=fields['group_name'], response_code=1, message='%s group resources delete "%s" failed - %s.' % (lno('GV13'), fields['group_name'], msg), user_groups=user_groups, attributes=columns)
        ...
        if rc != 0:
            config.db_close()
            return list(request, selector=fields['group_name'], response_code=1, message='%s group resource metadata delete "%s" failed - %s.' % (lno('GV14'), fields['group_name'], msg), user_groups=user_groups, attributes=columns)
        ...
        if rc != 0:
            config.db_close()
            return list(request, selector=fields['group_name'], response_code=1, message='%s delete group metadata exclusions for group "%s" failed - %s.' % (lno('GV14'), fields['group_name'], msg), user_groups=user_groups, attributes=columns)
        ...
        if rc != 0:
            config.db_close()
            return list(request, selector=fields['group_name'], response_code=1, message='%s group users delete "%s" failed - %s.' % (lno('GV15'), fields['group_name'], msg), user_groups=user_groups, attributes=columns)
        ...
        if rc != 0:
            config.db_close()
            return list(request, selector=fields['group_name'], response_code=1, message='%s group VMs defaults delete "%s" failed - %s.' % (lno('GV16'), fields['group_name'], msg), user_groups=user_groups, attributes=columns)
        ...
        if rc != 0:
            config.db_close()
            return list(request, selector=fields['group_name'], response_code=1, message='%s group keynames delete "%s" failed - %s.' % (lno('GV17'), fields['group_name'], msg), user_groups=user_groups, attributes=columns)
        ...
        if rc != 0:
            config.db_close()
            return list(request, selector=fields['group_name'], response_code=1, message='%s group limits delete "%s" failed - %s.' % (lno('GV18'), fields['group_name'], msg), user_groups=user_groups, attributes=columns)
        ...
        if rc != 0:
            config.db_close()
            return list(request, selector=fields['group_name'], response_code=1, message='%s group images delete "%s" failed - %s.' % (lno('GV19'), fields['group_name'], msg), user_groups=user_groups, attributes=columns)
        ...
        if rc != 0:
            return list(request, selector=fields['group_name'], response_code=1, message='%s group flavors delete "%s" failed - %s.' % (lno('GV20'), fields['group_name'], msg), user_groups=user_groups, attributes=columns)
        ...
        if rc == 0:
            config.db_close(commit=True)
            return list(request, selector=fields['group_name'], response_code=0, message='group "%s" successfully deleted.' % (fields['group_name']), user_groups=user_groups, attributes=columns)
        else:
            config.db_close()
            return list(request, selector=fields['group_name'], response_code=1, message='%s group delete "%s" failed - %s.' % (lno('GV21'), fields['group_name'], msg), user_groups=user_groups, attributes=columns)
...
def metadata_delete(request):
    ...
    if request.method == 'POST':
        ...
        if rc != 0:
            config.db_close()
            return list(request, selector=fields['group_name'], response_code=1, message='%s delete group metadata exclusion for group=%s, metadata=%s failed - %s.' % (lno('GV14'), fields['group_name'], fields['metadata_name'], msg), user_groups=user_groups, attributes=columns)
...
def update(request):
    ...
    if request.method == 'POST':
        if rc != 0:
            ...
            config.db_close()
            return list(request, selector='-', response_code=1, message='%s %s' % (lno('GV41'), msg), user_groups=user_groups)
        ...
        if rc != 0:
            active_user=active_user.username
            config.db_close()
            return list(request, selector='-', response_code=1, message='%s group update %s' % (lno('GV42'), msg), user_groups=user_groups)
        ...
        if 'username' in fields:
            ...
            if rc != 0:
                active_user=active_user.username
                return list(request, selector=fields['group_name'], response_code=1, message='%s group add, "%s" failed - %s.' % (lno('GV43'), fields['group_name'], msg), user_groups=user_groups)
        ...
        if len(group_updates) > 0:
            ...
            if rc != 0:
                active_user=active_user.username
                config.db_close()
                return list(request, selector=fields['group_name'], response_code=1, message='%s group update, "%s" failed - %s.' % (lno('GV44'), fields['group_name'], msg), user_groups=user_groups)
        else:
            if 'username' not in fields:
                active_user=active_user.username
                config.db_close()
                return list(request, selector=fields['group_name'], response_code=1, message='%s group update must specify at least one field to update.' % lno('GV45'), user_groups=user_groups)
        ...
        if rc == 0:
            active_user=active_user.username
            config.db_close(commit=True)
            return list(request, selector=fields['group_name'], response_code=0, message='group "%s" successfully updated.' % (fields['group_name']), user_groups=user_groups, attributes=columns)
        else:
            active_user=active_user.username
            config.db_close()
            return list(request, selector=fields['group_name'], response_code=1, message='%s group update "%s" failed - %s.' % (lno('GV46'), fields['group_name'], msg), user_groups=user_groups, attributes=columns)
