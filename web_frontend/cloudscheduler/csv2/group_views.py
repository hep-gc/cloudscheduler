...
def metadata_add(request):
    ...
    if request.method == 'POST' and 'metadata_name' in request.POST:
        ...
        if rc != 0:
            db_close(db_ctl)
            return render(request, 'csv2/groups.html', {'response_code': 1, 'message': '%s %s' % (lno('GV24'), msg)})
...
def metadata_fetch(request, selector=None):
    ...
    if rc != 0:
        db_close(db_ctl)
        return render(request, 'csv2/groups.html', {'response_code': 1, 'message': '%s %s' % (lno('GV34'), msg)})
