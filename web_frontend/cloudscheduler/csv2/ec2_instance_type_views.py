...
def foreign(request):
    ...
    if rc != 0:
        config.db_close()
        return render(request, 'csv2/foreign.html', {'response_code': 1, 'message': msg})
...
def list(request, args=None, response_code=0, message=None):
    ...
    if args==None:
        ...
        if rc != 0:
            config.db_close()
            return render(request, 'csv2/ec2_instance_types.html', {'response_code': 1, 'message': '%s vm list, %s' % (lno('VV00'), msg)})

    # Retrieve VM information.
    rc, msg, ec2_instance_type_list = qt(config.db_connection.execute(select_ec2_instance_types(config, active_user.active_group, fields['cloud_name'])))
    if rc != 0:
        config.db_close()
        return render(request, 'csv2/ec2_instance_types.html', {'response_code': 1, 'message': '%s vm list, %s' % (lno('VV00'), msg)})
    ...
    context = {
            ...
            'active_group': active_user.active_group,
            'user_groups': active_user.user_groups,
            'ec2_instance_type_list': ec2_instance_type_list,
...
def update(
    ...
    if rc != 0:
        config.db_close()
        return render(request, 'csv2/ec2_instance_types.html', {'response_code': 1, 'message': '%s %s' % (lno('VV01'), msg), 'active_user': active_user.username, 'active_group': active_user.active_group, 'user_groups': active_user.user_groups})
#       return list(request, selector, response_code=1, message='%s %s' % (lno('VV01'), msg), user_groups=user_groups)
    ...
    if request.method == 'POST':
        ...
        if rc != 0:
            config.db_close()
            return render(request, 'csv2/ec2_instance_types.html', {'response_code': 1, 'message': '%s vm update %s' % (lno('VV02'), msg), 'active_user': active_user.username, 'active_group': active_user.active_group, 'user_groups': active_user.user_groups})
#           return list(request, selector, response_code=1, message='%s vm update %s' % (lno('VV02'), msg), user_groups=user_groups)
        ...
        if fields['vm_option'] == 'kill':
            ...
            if fields['vm_hosts'].isnumeric():
                ...
                config.db_close()
                return render(request, 'csv2/ec2_instance_types.html', {'response_code': 1, 'message': '%s vm update, the "--vm-hosts" parameter must be numeric when "--vm-option retain" is specified.' % lno('VV98'), 'active_user': active_user.username, 'active_group': active_user.active_group, 'user_groups': active_user.user_groups})
#               return list(request, selector, response_code=1, message='%s vm update, the "--vm-hosts" parameter must be numeric when "--vm-option retain" is specified.' % lno('VV98'))
            ...
            return render(request, 'csv2/ec2_instance_types.html', {'response_code': 1, 'message': '%s vm update, option "%s" is invalid.' % (lno('VV03'), fields['vm_option']), 'active_user': active_user.username, 'active_group': active_user.active_group, 'user_groups': active_user.user_groups})
            ...
            if fields['vm_hosts'] != '':
                ...
                for vm in vm_list:
                    ...
                    if rc == 0:
                        ...
                        config.db_close()
                        return render(request, 'csv2/ec2_instance_types.html', {'response_code': 1, 'message': '%s vm update (%s) failed - %s' % (lno('VV04'), fields['vm_option'], msg), 'active_user': active_user.username, 'active_group': active_user.active_group, 'user_groups': active_user.user_groups})
#                       return list(request, selector, response_code=1, message='%s vm update (%s) failed - %s' % (lno('VV04'), fields['vm_option'], msg))
        ...
        #return render(request, 'csv2/ec2_instance_types.html', {'response_code': 0, 'message': 'vm update, VMs %s: %s.' % (verb, count), 'active_user': active_user.username, 'active_group': active_user.active_group, 'user_groups': active_user.user_groups})
    ...
    ### Bad request.
        ...
        return render(request, 'csv2/ec2_instance_types.html', {'response_code': 1, 'message': '%s vm update, invalid method "%s" specified.' % (lno('VV05'), request.method), 'active_user': active_user.username, 'active_group': active_user.active_group, 'user_groups': active_user.user_groups})
