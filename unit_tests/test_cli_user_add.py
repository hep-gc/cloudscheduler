...
def main(gvar):
    ...
    # 49 We have to specify server and server password explicitly because -SU has to be the last parameter.
    execute_csv2_command(
        gvar, 1, None, 'Value omitted for option: -SU | --super-user',
        ['user', 'add', '-s', 'unit-test', '-spw', gvar['user_secret'], '-un', ut_id(gvar, 'clu11'), '-upw', gvar['user_secret'], '-ucn', ut_id(gvar, 'command-line-user-11'), '-SU']
    )
