...
def setup():
    ...
    setup_commands = [
        ...
        ['alias', 'add', '-su', '{}-wig1'.format(gvar['user']), '-an', '{}-wia1'.format(gvar['user']), '-cn', '{}-wic1'.format(gvar['user'])],
        # Alias to be updated and deleted.
        ['alias', 'add', '-su', '{}-wig1'.format(gvar['user']), '-an', '{}-wia2'.format(gvar['user']), '-cn', '{}-wic1'.format(gvar['user'])],
        # Cloud metadata that should always exist.
        ['cloud', 'metadata-load', '-su', '{}-wig1'.format(gvar['user']), '-mn', '{}-wicm1'.format(gvar['user']), '-cn', '{}-wic1'.format(gvar['user']), '-f', NON_YAML_METADATA_FILE],
        # Cloud metadata to be deleted.
        ['cloud', 'metadata-load', '-su', '{}-wig1'.format(gvar['user']), '-mn', '{}-wicm2'.format(gvar['user']), '-cn', '{}-wic1'.format(gvar['user']), '-f', NON_YAML_METADATA_FILE],
        # Cloud metadata to be updated.
        ['cloud', 'metadata-load', '-su', '{}-wig1'.format(gvar['user']), '-mn', '{}-wicm3'.format(gvar['user']), '-cn', '{}-wic1'.format(gvar['user']), '-f', NON_YAML_METADATA_FILE],
        # Group metadata that should always exist.
        ['group', 'metadata-load', '-su', '{}-wig1'.format(gvar['user']), '-mn', '{}-wigm1'.format(gvar['user']), '-f', NON_YAML_METADATA_FILE],
        # Group metadata to be deleted.
        ['group', 'metadata-load', '-su', '{}-wig1'.format(gvar['user']), '-mn', '{}-wigm2'.format(gvar['user']), '-f', NON_YAML_METADATA_FILE],
        # Group metadata to be updated.
        ['group', 'metadata-load', '-su', '{}-wig1'.format(gvar['user']), '-mn', '{}-wigm3'.format(gvar['user']), '-f', NON_YAML_METADATA_FILE]
    ]
    ...
    if setup_required(set_to=False):
        ...
        for command in setup_commands:
            try:
                ...
                raise Exception('Error setting up tests.\ncmd={}\nstderr={}\nstdout={}'.format(err.cmd, err.stderr, err.stdout))
            print('.', end='')
...
def cleanup(gvar):
    ...
    for command in cleanup_commands:
        ...
        if process.returncode > 1:
            raise Exception('Error cleaning up tests.\ncmd={}\nstderr={}\nstdout={}'.format(' '.join(process.args), process.stderr, process.stdout))
