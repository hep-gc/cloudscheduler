...
def main(gvar):
    ...
    parameters = {
        ...
        # 10
        'vm_option': {'valid': 'kill', 'test_cases': {'invalid-unit-test': 'vm update value specified for "vm_option" must be one of the following options: [\'kill\', \'manctl\', \'retain\', \'retire\', \'sysctl\'].'}, 'mandatory': True},
        # 11 Give two vm_hosts.
        # 12
        'vm_hosts': {'valid': 'invalid-unit-test', 'test_cases': {'': 'vm update value specified for "vm_hosts" must not be the empty string'}, 'array_field': True},
    ...
    execute_csv2_request(
        gvar, 1, None, 'the "--vm-hosts" parameter must be numeric when "--vm-option retain" is specified', '/vm/update/',  group=ut_id(gvar, 'vtg1'), form_data={'vm_option': 'retain', 'vm_hosts.1': 'invalid-unit-test', 'vm_hosts.2': 'invalid-unit-test'}, server_user=ut_id(gvar, 'vtu1')
    )
