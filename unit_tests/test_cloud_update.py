...
def main(gvar):
    ...
    parameters = {
        ...
        # 44 give two vm_boot_volume_types
        # 45
    ...
    execute_csv2_request(
        ...
        '/cloud/update/', group=ut_id(gvar, 'ctg1'), form_data={'cloud_name': ut_id(gvar, 'csc3'), 'vm_boot_volume_type': 'invalid-unit-test', 'vm_boot_volume_size': 20},
    ...
    execute_csv2_request(
        gvar, 1, None, 'specified item does not exist: vm_boot_volume_type=invalid-unit-test',
        '/cloud/update/', group=ut_id(gvar, 'ctg1'), form_data={'cloud_name': ut_id(gvar, 'csc3'), 'vm_boot_volume_type': 'invalid-unit-test', 'vm_boot_volume_per_core': 20},
        server_user=ut_id(gvar, 'ctu1')
    )

    # 48 Ensure that metadata_option by itself does not qualify as a field to update.
    execute_csv2_request(
    ...
    # 49 Attempt to implicitly add metadata that does not exist.
    execute_csv2_request(
    ...
    # 50 Attempt to delete non-existent metadata.
    execute_csv2_request(
        gvar, 1, 'CV', 'cloud update, "{}" failed - specified metadata_name "invalid-unit-test" does not exist.'.format(ut_id(gvar, 'ctc3')),
    ...
    # 51 Ensures that values are as expected before executing requests that are expected to succeed. Known to fail if run twice without setup / cleanup in between.
    execute_csv2_request(
        gvar, 0, None, None,
    ...
    # 52 Update several values and explicitly add one metadata.
    execute_csv2_request(
    ...
    # 53 Ensure that 48 updated correctly. Known to fail if one's cloud_credentials (in ~/cloudscheduler/unit_tests/credentials.yaml) are invalid.
    execute_csv2_request(
        gvar, 0, None, None,
        '/cloud/list/', group=ut_id(gvar, 'ctg1'), expected_list='cloud_list', list_filter={'group_name': ut_id(gvar, 'ctg1'), 'cloud_name': ut_id(gvar, 'ctc3')},
        values={
    ...
    # 54 Implicitly add one metadata.
    execute_csv2_request(
        gvar, 0, None, 'cloud "{}::{}" successfully updated.'.format(ut_id(gvar, 'ctg1'), ut_id(gvar, 'ctc3')),
        '/cloud/update/', group=ut_id(gvar, 'ctg1'), form_data={
            'cloud_name': ut_id(gvar, 'ctc3'),
    ...
    # 55 Ensure that 50 added metadata.
    execute_csv2_request(
        gvar, 0, None, None,
        '/cloud/list/', group=ut_id(gvar, 'ctg1'), expected_list='cloud_list', list_filter={'group_name': ut_id(gvar, 'ctg1'), 'cloud_name': ut_id(gvar, 'ctc3')},
    ...
    # 57 Ensure that 52 deleted metadata.
    execute_csv2_request(
        gvar, 0, None, None,
        '/cloud/list/', group=ut_id(gvar, 'ctg1'), expected_list='cloud_list', list_filter={'group_name': ut_id(gvar, 'ctg1'), 'cloud_name': ut_id(gvar, 'ctc3')},
    ...
    # 59 Ensure that 54 added metadata.
    execute_csv2_request(
        gvar, 0, None, None,
    ...
    # 61 Ensure that 58 added metadata.
    execute_csv2_request(
        gvar, 0, None, None,
        '/cloud/list/', group=ut_id(gvar, 'ctg1'), expected_list='cloud_list', list_filter={'group_name': ut_id(gvar, 'ctg1'), 'cloud_name': ut_id(gvar, 'ctc3')},
        values={'group_exclusions': ut_id(gvar, 'cty1,cty2')},
    ...
    execute_csv2_request(
        gvar, 0, None, 'cloud "{}::{}" successfully updated.'.format(ut_id(gvar, 'ctg1'), ut_id(gvar, 'ctc3')),
        '/cloud/update/', group=ut_id(gvar, 'ctg1'), form_data={'cloud_name': ut_id(gvar, 'ctc3'), 'vm_boot_volume_type': 'None'},
        server_user=ut_id(gvar, 'ctu1')
    )
