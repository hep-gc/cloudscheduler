...
def main(gvar, user_secret):
    ...
    execute_csv2_command(
        gvar, 0, None, 'defaults list, table #1 columns: keys=server, columns=cloud-address,cloud-enabled,cloud-name,cloud-password,cloud-project,cloud-project-domain,cloud-region,cloud-spot-price,cloud-type,cloud-user,cloud-user-domain,expose-API,file-path,group,group-manager,group-metadata-exclusion,group-metadata-option,group-name,group-option,help,job-cores,job-disk,job-hold,job-id,job-image,job-priority,job-ram,job-request-cpus,job-request-disk,job-request-ram,job-request-swap,job-requirements,job-status,job-swap,job-target-clouds,job-user,long-help,metadata-enabled,metadata-mime-type,metadata-name,metadata-priority,no-view,only-keys,rotate,server,server-address,server-grid-cert,server-grid-key,server-password,server-user,super-user,text-editor,user-common-name,user-option,user-password,username,view,view-columns,vm-cores,vm-disk,vm-flavor,vm-foreign,vm-hostname,vm-image,vm-keep-alive,vm-keypair,vm-option,vm-ram,vm-status,vm-swap,yes',
