...
def main(gvar, user_secret):
    ...
    # 17
    execute_csv2_command(
        gvar, 0, None, 'defaults list, 1. Defaults (default server = dev): keys=server, columns=alias-name,backup-key,backup-repository,cacerts,cloud-address,cloud-enabled,cloud-flavor-exclusion,cloud-flavor-option,cloud-name,cloud-option,cloud-password,cloud-project,cloud-project-domain,cloud-region,cloud-spot-price,cloud-type,cloud-user,cloud-user-domain,comma-separated-values,comma-separated-values-separator,config-category,default-server,default_job_group,delete_cycle_interval,enable_glint,expose-API,file-path,group,group-metadata-exclusion,group-metadata-option,group-name,group-option,help,htcondor-container-hostname,htcondor-fqdn,htcondor-users,job-cores,job-disk,job-hold,job-id,job-image,job-priority,job-ram,job-request-cpus,job-request-disk,job-request-ram,job-request-swap,job-requirements,job-status,job-swap,job-target-clouds,job-user,log_file,log_level,long-help,metadata-enabled,metadata-mime-type,metadata-name,metadata-priority,no-view,no_limit_default,only-keys,rotate,server,server-address,server-grid-cert,server-grid-key,server-password,server-user,sleep-interval-main-long,sleep-interval-main-short,sleep_interval_cleanup,sleep_interval_command,sleep_interval_flavor,sleep_interval_image,sleep_interval_job,sleep_interval_keypair,sleep_interval_limit,sleep_interval_machine,sleep_interval_network,sleep_interval_vm,status-global-switch,status-refresh-internal,super-user,text-editor,user-common-name,user-option,user-password,username,view,view-columns,vm-cores,vm-cores-softmax,vm-disk,vm-flavor,vm-foreign,vm-hosts,vm-image,vm-keep-alive,vm-keyname,vm-network,vm-option,vm-ram,vm-security-groups,vm-status,vm-swap,with,yes',
