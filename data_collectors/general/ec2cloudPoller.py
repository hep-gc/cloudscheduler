...
#
# Target data sets (should all be available from libcloud??):
#       Flavor information
#       Quota Information
#       Image Information
#       Network Information
    ...
    try:
        while True:
            ...
            for vm in vm_to_destroy:
                if foreign(vm):
                    logging.info("skipping foreign VM %s marked for termination... - %s::%s" % (
                    vm.hostname, vm.group_name, vm.cloud_name))
                    continue
            ...
            config.db_close(
                commit=True)  # may need to batch these commits if we are attempting to terminate a lot of vms at once or the database sesion will time out
            del db_session
            ...
            for cloud in unique_cloud_dict:
                ...
                for flavor in flav_list:
                    ...
                    for groups in unique_cloud_dict[cloud]['groups']:
                        ...
                        if test_and_set_inventory_item_hash(inventory, group_n, cloud_n, flavor.name, flav_dict,
                        ...
                        try:
                            ...
                            logging.exception("Failed to merge flavor entry for %s::%s::%s, aborting cycle..." % (
                            group_n, cloud_n, flavor.name))
                            logging.error(exc)
                            abort_cycle = True
                            break
        ...
        while True:
            ...
            for cloud in unique_cloud_dict:
                ...
                for groups in unique_cloud_dict[cloud]['groups']:
                    ...
                    if test_and_set_inventory_item_hash(inventory, group_n, cloud_n, '-', limits_dict, new_poll_time,
                                                        debug_hash=(config.log_level < 20)):
                        continue
            ...
            for cloud in unique_cloud_dict:
                ...
                for network in net_list:
                    for groups in unique_cloud_dict[cloud]['groups']:
                        ...
                        if test_and_set_inventory_item_hash(inventory, group_n, cloud_n, network['name'], network_dict,
                                                            new_poll_time, debug_hash=(config.log_level < 20)):
                            continue
                ...
                for cloud in cloud_list:
                    ...
                    if session is False:
                        ...
                        if failure_dict[group_name + cloud_name] > 3:  # should be configurable
                            logging.error("Failure threshhold limit reached for %s, manual action required, exiting" % (
                            group_name, cloud_name))
                            return False
                        continue
                    ...
                    try:
                        ...
                        if failure_dict[group_name + cloud_name] > 3:  # should be configurable
                            logging.error(
                                "Failure threshhold limit reached for %s::%s, manual action required, exiting" % (
                                group_name, cloud_name))
                            return False
                        continue
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            if orange:
                previous_count, current_count = set_orange_count(logging, config, 'csv2_openstack_error_count',
